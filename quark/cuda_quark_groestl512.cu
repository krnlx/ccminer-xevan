...
//#define SPH_T64(x)    ((x) & (0xFFFFFFFFFFFFFFFFUL))
...
__global__ __launch_bounds__(TPB50, 2)
#endif
void quark_groestl512_gpu_hash_512(uint32_t threads, uint64_t* g_hash){
...
__host__
void quark_groestl512(int thr_id, uint32_t threads, uint32_t *d_hash){
        ...
        dim3 block(tpb);
        quark_groestl512_gpu_hash_512<<<grid, block>>>(threads, (uint64_t*)d_hash);
}
...
__constant__ static const sph_u64 SKEIN_IV512[] = {
...
#define TPB_SKEIN 512
__global__ __launch_bounds__(TPB_SKEIN, 2)
void xevan_skein512_gpu(uint32_t threads, uint64_t* g_hash){
...
*/
		uint64_t hash[8];
                *(uint2x4*)&hash[0] = __ldg4((uint2x4*)&inpHash[0]);
                *(uint2x4*)&hash[4] = __ldg4((uint2x4*)&inpHash[4]);
...
/*
  ...
  hash[6]=h6;
  hash[7]=h7;
        *(uint2x4*)&inpHash[ 0] = *(uint2x4*)&hash[ 0];
        *(uint2x4*)&inpHash[ 4] = *(uint2x4*)&hash[ 4];
*/
...
__host__
void xevan_skein512(int thr_id, uint32_t threads, uint32_t *d_hash){

        const int dev_id = device_map[thr_id];
        uint32_t tpb = TPB_SKEIN;

        dim3 grid(threads/tpb);
        dim3 block(tpb);
        xevan_skein512_gpu<<<grid, block>>>(threads, (uint64_t*)d_hash);
}


__device__ __forceinline__
void groestl512_progressMessage_quad1(uint32_t *state, uint32_t *message)
{
#pragma unroll 8
...
//if ((threadIdx.x & 0x03) == 3) msgBitsliced[ 1] ^= 0x00008000;
...
for (unsigned int u = 0; u < 8; u++)
    gBitsliced[u] = m2Bitsliced[u] ^ hBitsliced[u];
                ...
                groestl512_perm_P_quad(gBitsliced);
//                groestl512_perm_Q_quad(m2Bitsliced);

/*
if ((blockDim.x * blockIdx.x + threadIdx.x)  == 4) {
for(int i=0;i<8;i++)
	printf("m2Bitsliced[%i]=0x%x;\n",i,m2Bitsliced[i]);
